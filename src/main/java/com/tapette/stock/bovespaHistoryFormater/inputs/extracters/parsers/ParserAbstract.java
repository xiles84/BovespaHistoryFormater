...
public abstract class ParserAbstract implements Parsers{
	...
	@Override
	public abstract StockEntry parseTags(String str, Stock stock) throws NumberFormatException, ExceptionInvalidFormat ;
	
	@Override
	public abstract URL getStockURL(Stock stock) throws MalformedURLException;
	
	protected String transformBrazilDateIntoUniversalDate(String brazilDate) throws ExceptionInvalidFormat {
		if(!brazilDate.contains("/")) {
			if(brazilDate.length() == 8)
				return brazilDate;
			throw new ExceptionInvalidFormat("Date [" + brazilDate + "] does not contain \"/\"");
		}
		String[] list = brazilDate.split("/");
		if(list.length < 3)
			throw new ExceptionInvalidFormat("Date [" + brazilDate + "] does not have the expected format");
